...
<?php
	if(isset($_GET["id"])){
		...
		$result = mysql_query($conn ,$sqlstring);
...
<?php
	if(isset($_POST["bntUpdate"]))
	{
			...
			if($pic['type']=="image/jpg" || $pic['type']=="image/jpeg" || $pic['type']=="image/png" || $pic['type']=="image/gif"){
				if($pic['size']<= 614400)
				{
					...
					if(mysqli_num_rows($result)==0)
					{
						...
						$sqlstring ="UPDATE product set Product_Name = '$proname', Price=$price, SmallDesc='$short', DetailDesc='$detail', Pro_qty=$qty, Pro_image='$filePic', Cat_ID='$category', ProDate='".date('Y-m-d H:i:s')."' WHERE Product_ID='$id'";
		...
		else
		{
			...
			if(mysqli_num_rows($result)==0)
			{
				$sqlstring = "UPDATE product SET Product_Name = '$proname', Price = $price, SmallDesc='$short', DetailDesc='$detail', Pro_qty = $qty, Cat_ID='$category', ProDate = '".date('Y-m-d H:i:s')."' WHERE Product_ID='$id'";
